    ...
    for ($i = 1,$h=1; $i <= $data->sheets[$x]['numRows']; $i++,$h++) {// defino $i=2 para q no tome la primera fila q es la cabecera
                ...
                for ($j = 1; $j <= $data->sheets[$x]['numCols']; $j++) {
					...
					if($j==$data->sheets[$x]['numCols']){
						...
						$flag_cta[3][$s]="select idcuenta from cuentas where idcuenta='".$data->sheets[$x]['cells'][$i][3]."-1'";
						$update_cta[3][$s]="update cuentas set idproducto=522,idtipocartera=1 where idcuenta='".$data->sheets[$x]['cells'][$i][3]."-1'";
						$cuentas[$s]="INSERT INTO cuentas (idmoneda,idproducto,idcartera,idusuario,idtipocartera,idcliente,idcuenta)
							values (1,522,51,2,1,'".$data->sheets[$x]['cells'][$i][2]."','".$data->sheets[$x]['cells'][$i][3]."-1');
							";
						...
						$flag_cta[4][$s]="select idcuenta from cuenta_periodos where idcuenta='".$data->sheets[$x]['cells'][$i][3]."-1' and idperiodo=22";
						$update_cta[4][$s]="update cuenta_periodos set grupo=".$data->sheets[$x]['cells'][$i][33].",fecven='".$fecven."',impmin=".$data->sheets[$x]['cells'][$i][35].",imptot=".$data->sheets[$x]['cells'][$i][37].",diasmora='".$data->sheets[$x]['cells'][$i][40]."',ciclo=".$data->sheets[$x]['cells'][$i][39].",observacion2='$riesgo',observacion='".$data->sheets[$x]['cells'][$i][1]."-".$data->sheets[$x]['cells'][$i][3]."-".$data->sheets[$x]['cells'][$i][4]."' where idcuenta='".$data->sheets[$x]['cells'][$i][3]."-1' and idperiodo=22";
						$cuentas_p[$s]="INSERT INTO cuenta_periodos (idperiodo,idusuario,idcuenta,grupo,fecven,impmin,imptot,diasmora,ciclo,observacion2,observacion)
							values (22,2,'".$data->sheets[$x]['cells'][$i][3]."-1','".$data->sheets[$x]['cells'][$i][33]."','".$fecven."','".$data->sheets[$x]['cells'][$i][35]."','".$data->sheets[$x]['cells'][$i][37]."','".$data->sheets[$x]['cells'][$i][40]."','".$data->sheets[$x]['cells'][$i][39]."','".$riesgo."','".$data->sheets[$x]['cells'][$i][1]."-".$data->sheets[$x]['cells'][$i][3]."-".$data->sheets[$x]['cells'][$i][4]."');
							";
