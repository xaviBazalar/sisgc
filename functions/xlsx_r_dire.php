...
/*Inicio Instrucciones*/
	$objPHPExcel->getProperties()->setCreator("Kobsa - Gestion")
		...
		/*$sql="SELECT MIN(j.peso),c.idcliente,c.cliente,d.direccion,d.coddpto,d.codprov,d.coddist,od.origendireccion,v.validaciones,f.fuente,
				d.fecreg,u.ubicabilidad,a.actividad,gg.grupogestion,r.resultado,tp.tipo_predio,mp.material,np.piso,cpr.color,g.observacion,
				g.fecreg,g.fecges fbest
					FROM clientes c 
					JOIN cuentas ct ON c.idcliente=ct.idcliente
					JOIN gestiones g ON ct.idcuenta=g.idcuenta AND g.idagente
					...
					LEFT JOIN fuentes f ON d.idfuente=f.idfuente
					LEFT JOIN actividades a ON g.idactividad=a.idactividad
					...
					LEFT JOIN ubicabilidad u ON g.idubicabilidad=u.idubicabilidad
					WHERE ct.idcartera='15'
						 ";*/
		$sql="SELECT MIN(j.peso),c.idcliente,c.cliente,d.direccion,d.coddpto,d.codprov,d.coddist,od.origendireccion,v.validaciones,f.fuente,
				d.fecreg,u.ubicabilidad,a.actividad,gg.grupogestion,r.resultado,tp.tipo_predio,mp.material,np.piso,cpr.color,g.observacion,
				g.fecreg,g.fecges fbest
					...
					LEFT JOIN gestiones g ON d.iddireccion=g.iddireccion AND g.idagente
					JOIN carteras cr ON ct.idcartera=cr.idcartera
					JOIN origen_direcciones od ON d.idorigendireccion=od.idorigendireccion
					LEFT JOIN validaciones v ON d.idvalidacion=v.idvalidaciones
					LEFT JOIN fuentes f ON d.idfuente=f.idfuente
		...
		$sql.=" GROUP BY d.direccion ORDER BY c.idcliente ";
		/*echo $sql;
		return false;*/
