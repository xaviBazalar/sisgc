...
if(isset($_GET['value'])){
	...
	//<input disabled size='52' style='color:#000;background-color: #F9FCFF;border:none;font-size:11px;' type='text' value='".$dir->fields['dpto']."-".$dir->fields['prov']."-".$dir->fields['dist']."'/>
		...
		echo "<span id=\"span_gestiones_campo\" style=\"visibility:visible;position:relative\">
											...
											$query=$db->Execute("	SELECT tp.tipo_predio,mp.material,np.piso,clp.color,g.observacion,g.idcuenta,us.usuario,n.nivel,g.fecges,g.horges,g.fecreg,g.idgestion,cu.idcuenta,c.contactabilidad,r.resultado,g.feccomp,g.impcomp,d.direccion,g.observacion FROM gestiones g
																	JOIN resultados r ON g.idresultado=r.idresultado
																	LEFT JOIN direcciones d ON g.iddireccion=d.iddireccion
																	JOIN contactabilidad c ON g.idcontactabilidad=c.idcontactabilidad
																	JOIN cuentas cu ON g.idcuenta=cu.idcuenta
																	LEFT JOIN usuarios us ON g.usureg=us.idusuario
																	...
																	LEFT JOIN colores_pared clp ON g.idcolor_pared=clp.idcolor_pared
																	WHERE cu.idcliente='$idc'  
																	-- AND us.idnivel='3' 
																	AND g.idagente
																	AND g.idactividad=4
																	GROUP BY g.fecreg ORDER BY fecges DESC, horges DESC 
																	Limit 0,10
																	");						
																	//AND us.idnivel=3
											...
											while(!$query->EOF){
												echo 
													"<tr >
														...
														<td >".
															$query->fields['tipo_predio']
															."-".
															$query->fields['material']
															."-".
															$query->fields['piso']
															."-".
															$query->fields['color']
...
$id=$_GET['id_gca'];
$result = $db->Execute("SELECT iddireccion,direccion,
						...
						(SELECT nombre FROM ubigeos WHERE coddpto=d.coddpto AND codprov=d.codprov AND coddist=d.coddist) dist
						  FROM direcciones d WHERE idcliente='$id'
  							-- AND coddpto!=0 AND codprov!=0
							");
	while(!$result->EOF){
		$direccion_id=$result->fields['direccion'];
		if(strpos($direccion_id,"/"))
		{
			$direccion_id=str_replace("/"," ",$direccion_id);
		}
		$ubigeo=$result->fields['dpto']."-".$result->fields['prov']."-".$result->fields['dist'];
		echo $result->fields['iddireccion']."/".$direccion_id." $ubigeo*";
		$result->MoveNext();
	}
	mysql_free_result($result->_queryID);
echo	"<br/>";
