...
include '../scripts/conexion.php';
		...
		$sql="SELECT ct.feccon,gt.idgestion,ct.feccon,MIN(j.peso) peso,MAX(gt.fecreg),cl.idcliente,cl.cliente,cp.idcuenta
				...
				gt.observacion obs_ges,tl.telefono,r.idresultado,gt.idjustificacion,
				CASE 
				    WHEN gg.idgrupogestion='7' THEN 'CD'
				    WHEN gg.idgrupogestion='8' THEN 'CI'
				    WHEN gg.idgrupogestion='4' or  gg.idgrupogestion='9' THEN 'NC'
		...
		$sql.="
				AND gt.peso=(
				SELECT MIN(g.peso) 
				FROM gestiones g WHERE g.idcuenta=gt.idcuenta)"	;
		...
		$titulo="NUMERO_CUENTA\tNOMBRE_CALL\tNOMBRE_BASE\tDOCUMENTO\tNOMBRE_COMPLETO\tFECHA DE ALTA\tFECHA DE VENC\tDISTRITO\tPROVINCIA\tDEPARTAMENTO\tCODIGO_TDA\tNOM_TDA\tDERIVAR A :\tJefe _Agencia\tRegional\tTELEFONO_1\tTELEFONO_2\tTELEFONO_3\tTELEFONO_4\tTELEFONO_5\tCAMPO_LIBRE_1\tCAMPO_LIBRE_2\tTLF_ADIC_1\tTLF_ADIC_2\tEMAIL\tOBS_BD\tF_CONTACTO\tHRA_CONTACTO\tTIPO_CONTACTO\tRESULTADO\tDETALLE\tTV\tQ_CONTACTOS\tQ_MARCACIONES\tF_ASISTENCIA\tHORA_ASISTENCIA";

		$fp = fopen('f_tv_tn.txt', 'w');
		...
		$query=$db->Execute($sql);
				...
				while(!$query->EOF){
					...
					$pos = strpos($query->fields['idcuenta'], "-");
							...
							$cont.=$tp_push[0]."-".$tp_push[2]."-".$tp_push[3]."\t";
							...
							if($query->fields['idjustificacion']==226){
								$cita=explode("*",$query->fields['obs_ges']);

								$f_ct=str_replace("(","",$cita[0]);
								$h_ct=$cita[1];
								$cont.=$f_ct."\t";
								$cont.=$h_ct."\t";
							}else{
				...
				while(!$query->EOF){
					...
					$pos = strpos($query->fields['idcuenta'], "-");
							...
							if($query->fields['idtipocartera']==36){
								$tp_push=explode("-",$query->fields['observacion']);
								$cont.="\t";
								$cont.=$tp_push[1]."\t";
			...
			echo "Foto TV:<a href='guardar_como.php?name=f_tv_tn.txt' target='blank'>Click para descargar</a><br/>";	
