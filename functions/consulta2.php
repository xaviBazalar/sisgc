...
<table border="0" id="design1" width="100%">
				...
				<tbody>
						...
						$dom = new DOMDocument();
						$dom->load( 'xml_user/xml_'.$_SESSION['iduser'].'.xml' );	
						...
						foreach ($filas as $fila){
							...
							echo"<td>$s</td>";
								if($_SESSION['tnivel']=="2" or $_SESSION['tnivel']=="3"){
									echo "<td align='center'><a href='index.php?gestion=1&idCl=".$fila->getElementsByTagName( "idcliente" )->item(0)->nodeValue."&mn=$s";
									echo "'>".$fila->getElementsByTagName( "idcliente" )->item(0)->nodeValue."</a></td>";
									$cliente=$fila->getElementsByTagName( "idcliente" )->item(0)->nodeValue;
								}else{
									echo "<td align='center'><a href='index.php?gestion_c=1&idCl=".$fila->getElementsByTagName( "idcliente" )->item(0)->nodeValue."'>".$fila->getElementsByTagName( "idcliente" )->item(0)->nodeValue."</a></td>";
								}
                            echo "<td>".utf8_encode($fila->getElementsByTagName( "cliente" )->item(0)->nodeValue)."</td>";
							echo "<td>".$fila->getElementsByTagName( "proveedor" )->item(0)->nodeValue."</td>";
                            echo "<td>".$fila->getElementsByTagName( "moneda" )->item(0)->nodeValue."</td>";
							$sp= explode(".",$fila->getElementsByTagName( "imptot" )->item(0)->nodeValue);
																		$tot_impt=strlen($sp[0]);
																		if($tot_impt>=4){
																			$imptot=substr_replace($sp[0],',',-3,0).".".$sp[1];
																		}else{
																			$imptot=$fila->getElementsByTagName( "imptot" )->item(0)->nodeValue;
																		}	
                            echo "<td align='center'>".$imptot."</td>";
							
                            echo "<td>".$fila->getElementsByTagName( "diasmora" )->item(0)->nodeValue."</td>";
                            echo "<td>".$fila->getElementsByTagName( "grupo" )->item(0)->nodeValue."</td>";
							echo "<td>".$fila->getElementsByTagName( "ciclo" )->item(0)->nodeValue."</td>";
							echo "<td align='center'>".$fila->getElementsByTagName( "fec_ges_max" )->item(0)->nodeValue."</td></tr>";
							
							$_SESSION['next'][$s]=$fila->getElementsByTagName( "idcliente" )->item(0)->nodeValue;
