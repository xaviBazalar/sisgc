...
$db_t=  $db;
	...
	if($_GET['tcart']!=""){ 
		$fil_c=" where c.idtipocartera='".$_GET['tcart']."' ";
		$fil_c2=" and c.idtipocartera='".$_GET['tcart']."' ";
	}else{
		$fil_c="";
		$fil_c2="";
	}
	...
	if(isset($_GET['idusuario']) and isset($_GET['idcartera'])){
		//$db->debug=true;
			if($_GET['liberar']=="1" and $_GET['idusuario']!="all"){
				...
				$fil_all="and idusuario=".$_GET['idusuario'];
			}else{
				$fil_lb="";
				$fil_all="";
			}
			$cuentas=$db->Execute("SELECT rs.idcuenta  FROM
									...
									FROM cuentas c
									JOIN cuenta_periodos cp ON c.idcuenta=cp.idcuenta AND cp.idperiodo=$id_periodo AND cp.idestado=1 AND cp.idusuario!=2
			...
			while(!$cuentas->EOF){
					...
					$sql_up_lb="Update cuenta_periodos set idusuario=2 where idcuenta='".$id_cta."' and idperiodo=$id_periodo $fil_all ";
	...
	$tot=$db->Execute("SELECT COUNT(DISTINCT(c.idcuenta)) total FROM cuentas c
		...
		$tot=$db->Execute("SELECT rs.idresultado,rs.resultado,count(*) total FROM
							...
							GROUP BY c.idcuenta) AS rs where rs.idresultado=".$_GET['idresultado']." GROUP BY rs.idresultado
							");
		$tot2=$db->Execute("SELECT rs.idresultado,rs.resultado,count(*) total FROM
							(SELECT cp.idcuenta,g.idresultado,r.resultado
							FROM cuentas c
							JOIN cuenta_periodos cp ON c.idcuenta=cp.idcuenta AND cp.idperiodo=$id_periodo AND cp.idestado=1  AND cp.idusuario=2
							JOIN gestiones g ON g.idcuenta=cp.idcuenta AND g.fecges LIKE '".date("Y-m")."%'
	...
	if($tot->fields['total']==""){
		$tot_total=0;
	}else{$tot_total=$tot->fields['total'];}

	if($tot2->fields['total']==""){
		$tot2_total=0;
	}else{$tot2_total=$tot2->fields['total'];}

	echo "Cuenta Filtradas : ".$tot_total." | Disponibles : ".$tot2_total."<br/>";
				...
				while(!$up->EOF){
					++$n;
						...
						$t_cta="SELECT cp.idusuario,COUNT(distinct(c.idcuenta)) tot FROM cuentas c
								...
								join resultados r on g.idresultado=r.idresultado 
								WHERE cp.idusuario=".$up->fields['idusuario']." AND c.idcartera=$id_cartera $fil_c2
