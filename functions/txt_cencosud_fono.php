...
$archivo="telAE7_$fecha.txt";
$fp = fopen($archivo, 'w');
	
	$db2= $db;
	$db=  $db_li;
				
	$sql="CALL txt_fono_reg700('$fecha_g','$id_periodo')";
	...
	while(!$rp->EOF){
		...
		if($total==91){
			...
			$db2->Execute("insert into debug_interfaces (idinterface,linea,longitud,fecha) values (3,'$linea',$total,'$fecha_g')");
		}
		$rp->MoveNext();
