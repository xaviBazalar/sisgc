	...
	$objPHPExcel = new PHPExcel();
	 ...
	 // Add some data
		...
		$objPHPExcel->setActiveSheetIndex(0)
							...
							->setCellValue('b3', "GESTION DE CONTACTO")
		...
		switch($_GET['clasi']){
			case 0:
				$riesgo_nom="Normal";
				$fil_ris=" and cp.observacion='0' ";
				$fil_ris_x=" and cp2.observacion='0' ";
				$tipo_ciber=5;
				break;
			case 1:
				$riesgo_nom="CPP";
				$fil_ris=" and cp.observacion='1' ";
				$fil_ris_x=" and cp2.observacion='1' ";
				$tipo_ciber=6;
				break;
			case 2:
				$riesgo_nom="Deficiente";
				$fil_ris=" and cp.observacion='2' ";
				$fil_ris_x=" and cp2.observacion='2' ";
				$tipo_ciber=7;
				break;
			case 3:
				$riesgo_nom="Dudoso";
				$fil_ris=" and cp.observacion='3' ";
				$fil_ris_x=" and cp2.observacion='3' ";
				$tipo_ciber=8;
				break;
			case 4:
				$riesgo_nom="Perdida";
				$fil_ris=" and cp.observacion='4' ";
				$fil_ris_x=" and cp2.observacion='4' ";
				$tipo_ciber=9;
				break;
		}
		//$db->debug=true;
		for($i=1;$i<=getMonthDays($mes, $ano);$i++){	
			...
			if($fecha_hoy!=$fecha){
				$sql="SELECT COUNT(*) tot_llamadas FROM (
							...
							JOIN cuenta_periodos cp ON c.idcuenta=cp.idcuenta AND cp.idperiodo=$peri and cp.idusuario!=1490 $fil_ris
			...
			$sql="SELECT
						/*(SELECT COUNT(*) FROM (
							SELECT idgestion FROM cuentas c
							JOIN cuenta_periodos cp ON c.idcuenta=cp.idcuenta AND cp.idperiodo=$peri $fil_ris
							JOIN gestiones g ON cp.idcuenta=g.idcuenta AND g.fecges='$ano-$mes-$nro'
							WHERE c.idcartera=$cartera AND g.idactividad!=4
							GROUP BY g.idcuenta,g.fecreg
							) AS rs2) tot_llamadas,*/
					...
					(
						...
						AND r.peso_r=(
								...
								JOIN cuenta_periodos cp2 ON c2.idcuenta=cp2.idcuenta AND cp2.idperiodo=$peri $fil_ris_x
				...
				$sql="SELECT * FROM ciber_reportes WHERE fecha='$ano-$mes-$nro' and id_tipo_ciber=$tipo_ciber";
			...
			$q=$db->Execute($sql);
					...
					while(!$q->EOF){
						...
						$sql_ss=$db->Execute("SELECT fecha,tot_llamadas FROM ciber_reportes WHERE fecha='$ano-$mes-$nro' and id_tipo_ciber=$tipo_ciber");
						//$db->debug=true;
						if($dia_n=="Dom"){
							...
							if($sql_ss->fields['fecha']==""){
								...
								$ins=$db->Execute("UPDATE ciber_reportes SET 
												...
												$act_tt
												tot_clientes=".$q->fields['tot_clientes'].",
		...
		$objPHPExcel->getActiveSheet()->setTitle($riesgo_nom);
...
header('Content-Disposition: attachment;filename="reporte_clasificacion_cc.xls"');
