    ...
    for ($i = 1,$h=1; $i <= $data->sheets[$x]['numRows']; $i++,$h++) {// defino $i=2 para q no tome la primera fila q es la cabecera
                ...
                for ($j = 1; $j <= $data->sheets[$x]['numCols']; $j++) {
					...
					if($j==$data->sheets[$x]['numCols']){
						...
						if(trim($data->sheets[$x]['cells'][$i][29])!="" and trim($data->sheets[$x]['cells'][$i][29])!="NULL"){
						
							if(substr($data->sheets[$x]['cells'][$i][18],0,1)==9){$oriT=10;}else{$oriT=9;}
							$flag_cta[2][$l]="select idcliente from telefonos where idcliente='".trim($data->sheets[$x]['cells'][$i][2])."' and telefono='".trim($data->sheets[$x]['cells'][$i][29])."'";
							$update_cta[2][$l]="update telefonos set idorigentelefono=$oriT,observacion='".$data->sheets[$x]['cells'][$i][28]."' where idcliente='".$data->sheets[$x]['cells'][$i][2]."' and telefono='".trim($data->sheets[$x]['cells'][$i][29])."'";
						...
						$flag_cta[3][$s]="select idcuenta from cuentas where idcuenta='".trim($data->sheets[$x]['cells'][$i][3])."-1' and idcliente='".trim($data->sheets[$x]['cells'][$i][2])."' ";
						$update_cta[3][$s]="update cuentas set obs2='".$data->sheets[$x]['cells'][$i][42]."*".$data->sheets[$x]['cells'][$i][43]."*".$data->sheets[$x]['cells'][$i][44]."', idproducto=522,idtipocartera=1 where idcuenta='".trim($data->sheets[$x]['cells'][$i][3])."-1'";
						$cuentas[$s]="INSERT INTO cuentas (idmoneda,idproducto,idcartera,idusuario,idtipocartera,idcliente,idcuenta,obs2)
							values (1,522,51,2,1,'".$data->sheets[$x]['cells'][$i][2]."','".trim($data->sheets[$x]['cells'][$i][3])."-1','".$data->sheets[$x]['cells'][$i][42]."*".$data->sheets[$x]['cells'][$i][43]."*".$data->sheets[$x]['cells'][$i][44]."');
							";
						...
						$sl="select idcuenta,idusuario from cuenta_periodos where idcuenta='".trim($data->sheets[$x]['cells'][$i][3])."-1' and idperiodo=".($id_periodo-1);
						...
						$flag_cta[4][$s]="select idcuenta from cuenta_periodos where idcuenta='".trim($data->sheets[$x]['cells'][$i][3])."-1' and idperiodo=$id_periodo";
						$update_cta[4][$s]="update cuenta_periodos set grupo=".$data->sheets[$x]['cells'][$i][33].",fecven='".$fecven."',impmin=".$data->sheets[$x]['cells'][$i][35].",imptot=".$data->sheets[$x]['cells'][$i][37].",diasmora='".$data->sheets[$x]['cells'][$i][40]."',ciclo=".$data->sheets[$x]['cells'][$i][39].",observacion2='$riesgo',observacion='".$data->sheets[$x]['cells'][$i][1]."-".$data->sheets[$x]['cells'][$i][3]."-".$data->sheets[$x]['cells'][$i][4]."' where idcuenta='".trim($data->sheets[$x]['cells'][$i][3])."-1' and idperiodo=$id_periodo";
						$cuentas_p[$s]="INSERT INTO cuenta_periodos (usureg,idperiodo,idusuario,idcuenta,grupo,fecven,impmin,imptot,diasmora,ciclo,observacion2,observacion)
							values ($id_periodo,$id_periodo,$idusu,'".trim($data->sheets[$x]['cells'][$i][3])."-1','".$data->sheets[$x]['cells'][$i][33]."','".$fecven."','".$data->sheets[$x]['cells'][$i][35]."','".$data->sheets[$x]['cells'][$i][37]."','".$data->sheets[$x]['cells'][$i][40]."','".$data->sheets[$x]['cells'][$i][39]."','".$riesgo."','".$data->sheets[$x]['cells'][$i][1]."-".$data->sheets[$x]['cells'][$i][3]."-".$data->sheets[$x]['cells'][$i][4]."');
							";
