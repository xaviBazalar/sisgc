...
Zapatec.Form.prototype.initField = function(currentField){
	...
	currentField.zpValidators = Zapatec.Form.getTokens(currentField.className, " ");
...
Zapatec.Form.isHourValid = function(cadenaHora){
	var vhora = cadenaHora.substring(0, 2);
	var vminuto = cadenaHora.substring(3, 5);

	return (vhora < 24 && vminuto < 60);
}
...
Zapatec.Form.prototype.setCaretPosition1 = function(el, pos){
...
Zapatec.Form.prototype.keydown = function(evt, elm) {
	var charCode = this.getInputData(evt)[0];

	if(charCode != 9){
		this.setValueFromField(elm);
	}
	
	// this is IE workaround - IE catches nonalphanumeric keys only on keydown.
	if((/zpFormMask="([^"]+)"/).test(elm.className) && Zapatec.is_ie){
		return this.processCustomKeys(charCode, this.getCaretPosition(elm), elm);
	}
}
...
Zapatec.Form.prototype.validate = function(elm) {
	...
	var validators = Zapatec.Form.dataTypes;
	...
	if(isEmpty){
		if(isRequired){
			message = " ";
			//message = Zapatec.Form.hasFieldFeature(elm, "zpFormRequiredError") ? Zapatec.Form.getFieldFeature(elm, "zpFormRequiredError") : "This field is required";
			errors.push({field: elm, errorMessage: message, validator: 'zpFormRequired'});
		}
	} else {
		for(var validatorName in elm.zpValidators){
			if(validatorName == 'zpFormMask' && !isFilled){
				valid = false;

				var mask = Zapatec.Form.getFieldFeature(elm, "zpFormMask");
				message = " ";
	
				errors.push({field: elm, errorMessage: message, validator: "zpFormMask"});
			}

			if(Zapatec.Form.hasFieldFeature(elm, 'zpFormMask') && !isFilled){
				continue;
			}
			...
			if(typeof(validators[validatorName]) != 'undefined'){
				...
				if (validator.regex) {
					// Regex validation.
					valid &= validator.regex.test(Zapatec.Form.getFieldValue(elm));
				} else if (validator.func) {
					// Javascript function validation.
					valid &= validator.func(Zapatec.Form.getFieldValue(elm), Zapatec.Form.getFieldFeature(elm, validatorName));
				}

				if (!valid){
					message = Zapatec.Form.hasFieldFeature(elm, validatorName + "Error") ?
						Zapatec.Form.getFieldFeature(elm, validatorName + "Error") :
						validator.error;

					errors.push({'field': elm, 'errorMessage': message, 'validator': validator});
				}
			}
		}
	}
...
*/
Zapatec.Form.prototype.setCaretPosition = function(elm, startPos, endPos){
	var valLength = Zapatec.Form.getFieldValue(elm).length;
	...
	if(typeof(elm.createTextRange) == "object"){
		// IE
		var range = elm.createTextRange();
		range.moveStart("character", startPos);
		range.moveEnd("character", endPos - valLength);
		range.select();

		return true;
	} else if (typeof(elm.setSelectionRange) == 'function'){
...
Zapatec.Form.prototype.getSelectionStart = function(elm){
...
Zapatec.Form.prototype.getSelectionEnd = function(elm){
	if (typeof(elm.selectionEnd) != "undefined") {
		// mozilla and opera
		return elm.selectionEnd;
	} else if (document.selection) {
		// IE black magic
		return elm.value.length - Math.abs(
...
Zapatec.Form.prototype.setValueFromField = function(field){
	if ((/zpFormAllowed-(\S+)/).test(field.className)) {//TODO: use hasFeature() here
		var self = this;
		var notallowed = new RegExp('[^\\' + (RegExp.$1).split('').join('\\') + ']', 'g');

		setTimeout(
			function(){
				var selStart = self.getSelectionStart(field);
				var selEnd = self.getSelectionEnd(field);

				var val = Zapatec.Form.getFieldValue(field);
				
				val = val.replace(notallowed, "");

				Zapatec.Form.setFieldValue(field, val);
				self.setCaretPosition(field, selStart, selEnd);
			},
			1
		);
	}
...
*/
Zapatec.Form.getTokens = function(className, separator){
...
Zapatec.Form.hasFieldFeature = function(field, feature){
	if(typeof(feature) == 'undefined' || feature == null){
		return false;
	}

	if(typeof(field.zpValidators[feature]) == 'undefined'){
		return false;
	}
...
Zapatec.Form.getFieldFeature = function(field, feature){
	if(typeof(feature) == 'undefined' || feature == null){
		return false;
	}

	return field.zpValidators[feature]
}
